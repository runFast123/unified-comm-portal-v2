...
export async function GET(request: Request) {
  ...
  let verifyToken = process.env.INSTAGRAM_VERIFY_TOKEN || process.env.META_VERIFY_TOKEN || ''
  if (account) {
    const cfg = await getChannelConfig(account, 'instagram')
...
export async function POST(request: Request) {
  try {
    ...
    let items: Array<{ inbound: ReturnType<typeof parseInstagramInbound>; senderId?: string }> = []

    if (nativeAccount && metaSig) {
      // ── NATIVE Meta Instagram webhook (no relay) ────────────────────────
      nativeMode = true
      const cfg = await getChannelConfig(nativeAccount, 'instagram')
      const appSecret = cfg?.app_secret || process.env.INSTAGRAM_APP_SECRET || ''
      ...
      // NO expectedPageId here: for Instagram webhooks entry.id is the IG
      // professional-account id, NOT the linked Facebook Page id we store —
      // filtering on page_id would drop real messages.
      const relays = metaMessagingEnvelopeToRelays(envelope, nativeAccount)
      // Echoes / delivery receipts / attachment-only → ack + ignore.
      if (relays.length === 0) return NextResponse.json({ ok: true, ignored: true })
      items = relays.map((r) => ({ inbound: parseInstagramInbound(r), senderId: r.sender_id }))
      ...
      items = [{ inbound: parseInstagramInbound(parsed.data), senderId: parsed.data.sender_id }]
    }
    ...
    const processOne = async (
      inbound: ReturnType<typeof parseInstagramInbound>,
      senderId: string | undefined
    ): Promise<{ status: number; body: Record<string, unknown> }> => {
      // The sender IGSID lands in teams_chat_id.
      ...
      if (accountSettings.phase1_enabled) {
        after(() =>
          fetch(`${origin}/api/classify`, {
            method: 'POST',
            headers: aiHeaders,
            body: JSON.stringify({ message_id: message.id, message_text: messageText, channel: 'instagram', account_id: accountIdFixed }),
          }).then(() => undefined, () => undefined)
        )
      }
