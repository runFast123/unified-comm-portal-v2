/**
 * PATCH / DELETE /api/company-tags/:id
 *
 * Admin / company-admin only. Scoped to the caller's own company unless they
 * are super_admin. Tags are hard-deleted because the catalog is purely a UI
 * autocomplete/coloring source — `conversations.tags text[]` keeps the tag
 * strings independently, so removing the catalog row just means the chip
 * loses its color the next time it renders.
 */
