...
function buildWidgetJs(key: string, origin: string): string {
  ...
  var color='#16a34a', title='Chat with us', welcome='Hi! How can we help?', subtitle='', launcher='', position='right', fg='#fff';
  ...
  function loadConfig(){return fetch(API+'/config?key='+encodeURIComponent(KEY)).then(function(r){return r.ok?r.json():null;}).then(function(c){
    if(c){title=c.title||title;color=c.color||color;welcome=c.welcome_message||welcome;subtitle=c.subtitle||'';launcher=c.launcher_text||'';position=c.position==='left'?'left':'right';}
    configured=true;
  ...
  function begin(){if(welcome&&!body.children.length)addMsg(null,'outbound',welcome);startPolling();}
  function openPanel(){open=true;panel.classList.add('lcw-open');launchEl.style.display='none';
    if(!started){started=true;if(configured)begin();else loadConfig().then(begin);}
    input.focus();}
