...
interface NotificationsClientProps {
  /**
   * Account IDs the caller is allowed to operate on. Resolved server-side
   * from the caller's company. `null` = super_admin (no scope; allow all).
   * An empty array = caller has a company with zero accounts; in that case
   * we render the page but every scoped mutation becomes a no-op so we
   * never run an unscoped query against `notification_rules`.
   */
...
export default function NotificationsClient({ companyAccountIds }: NotificationsClientProps) {
  ...
  // Convenience flag: caller has a company but zero accounts. Any scoped
  // mutation in that state is a no-op because there are no rules they own.
  const noCompanyAccounts =
    companyAccountIds !== null && companyAccountIds.length === 0
    ...
    try {
      if (editingRule) {
        if (noCompanyAccounts) {
          setStatusMessage('Failed to update rule: no accounts in your company.')
          setSaving(false)
          return
        }
        // SECURITY: chain `.in('account_id', companyAccountIds)` so a
        // company_admin can only update rules tied to accounts in their
        // own company, even if they craft a request with someone else's
        // rule id. RLS should also enforce this, but we add the explicit
        ...
        if (companyAccountIds !== null) {
          updateQ = updateQ.in('account_id', companyAccountIds)
        }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      // SECURITY: scope delete to caller's company accounts.
      let deleteQ = supabase
        .from('notification_rules')
        .delete()
        .eq('id', id)
      if (companyAccountIds !== null) {
        deleteQ = deleteQ.in('account_id', companyAccountIds)
      }
  ...
  const toggleEnabled = async (id: string) => {
    ...
    if (noCompanyAccounts) {
      ...
      setStatusMessage('Failed to toggle rule: no accounts in your company.')
      return
    }
    // SECURITY: scope toggle to caller's company accounts.
    let toggleQ = supabase
      .from('notification_rules')
      .update({ is_active: newValue })
      .eq('id', id)
    if (companyAccountIds !== null) {
      toggleQ = toggleQ.in('account_id', companyAccountIds)
    }
  ...
  const handleSaveWebhook = async () => {
    ...
    setStatusMessage(null)
    try {
      // SECURITY: only iterate over rules belonging to the current company.
      // `rules` is already scoped via loadRules(), so this filter is mainly
      // about being explicit. The per-rule update is additionally scoped
      // by chaining `.in('account_id', companyAccountIds)` so even a stale
      // local state can't escape the tenant.
      if (noCompanyAccounts) {
        setStatusMessage('No accounts in your company to update.')
        setSavingWebhook(false)
        return
      }
      const slackRules = rules.filter((r) => r.notify_slack)
      for (const rule of slackRules) {
        let webhookQ = supabase
          .from('notification_rules')
          .update({ slack_webhook_url: slackWebhook || null })
          .eq('id', rule.id)
        if (companyAccountIds !== null) {
          webhookQ = webhookQ.in('account_id', companyAccountIds)
        }
        await webhookQ
      }
